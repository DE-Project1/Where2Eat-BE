...
def get_clusters_by_region():
    def fetch(district: str) -> List[Dict[str, Any]]:
        # 1) 해당 구의 모든 adm_dong_code 가져오기
        adm_dong_codes = [
            doc["adm_dong_code"]
            for doc in db["region_map"].find(
                {"district": district},
                {"adm_dong_code": 1, "_id": 0}
            )
        ]

        if not adm_dong_codes:
            return []

        # 2) place_info 에서 adm_dong_code가 일치하는 모든 place_id 가져오기
        place_ids = [
            doc["place_id"]
            for doc in db["place_info"].find(
                {"adm_dong_code": {"$in": adm_dong_codes}},
                {"place_id": 1, "_id": 0}
            )
        ]
        if not place_ids:
        ...
        # 3) reviews에서 place_id in 필터 후 → cluster 기준으로 그룹 + 중복 허용 place_id push
        pipeline = [
            {"$match": {"place_id": {"$in": place_ids}}},
            {
                "$group": {
                    ...
                    "place_ids": {"$push": "$place_id"}  # 중복 허용 (중요)
                }
            },
            {
                "$project": {
                    ...
                    "places_count": {"$size": "$place_ids"},  # push라서 중복 허용한 count
                    "_id": 0,
                }
            }
        ]
...
def get_places_by_region_and_cluster():
    def fetch(district: str, cluster_id: int) -> List[Dict[str, Any]]:
        # 1) 해당 구의 모든 adm_dong_code 가져오기
        adm_dong_codes = [
            doc["adm_dong_code"]
            for doc in db["region_map"].find(
        ...
        # 3) reviews에서 adm_dong_code + cluster_id + place_id 로 필터 → 중복 허용
        matched_place_ids_raw = [
            doc["place_id"]
        ...
        if not matched_place_ids_raw:
            return []
        # 중복 제거 (set)
        matched_place_ids = list(set(matched_place_ids_raw))
...
def get_situation_definitions_by_place_id():
    def fetch(place_id: int, page: int = 1) -> List[Dict[str, Any]]:
        # 페이지 계산
        skip = (page - 1) * 10
